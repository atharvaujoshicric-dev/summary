...
def extract_area_logic(text):
    ...
    text = re.sub(r'[मक]हणज[चछ]े', 'म्हणजे', text) # Catch typos like म्हणजचे or कहणजचे
    ...
    start_keywords = r'(?:इमारतीमधील|गृहप्रकल्पातील|प्रकल्पातील|योजनेतील|नियोजित|प्रोजेक्ट|प्रोजेक्टमधील|बिल्डींग|इमारत|मिळकतीवर|मिळकतीवरील)'
    parts = re.split(start_keywords, text, flags=re.IGNORECASE)
    relevant_text = " ".join(parts[1:]) if len(parts) > 1 else text

    # 4. TOTAL AREA EXPLICIT PATTERN (असे एकूण क्षेत्र)
    total_pattern = rf'(?:एकूण\s*क्षेत्र|total\s*area)\s*(?:क्षेत्र\s*)?(\d+\.?\d*)\s*{m_unit}'
    total_match = re.search(total_pattern, relevant_text, re.IGNORECASE)
    if total_match:
        return round(float(total_match.group(1)), 3)

    # 5. METRIC SUMMATION (Collecting Area Components)
    exclude_keywords = ["पार्किंग", "पार्कींग", "parking", "road", "reserve", "राखीव", "प्लॉट", "plot", "साईज", "size", "बिल्डअप", "मुल्यांकन"]
    m_vals = []
    ...
    for match in re.finditer(rf'(\d+\.?\d*)\s*{m_unit}', relevant_text, re.IGNORECASE):
        ...
        # Avoid double-counting RERA duplicates in brackets
        bracket_context = relevant_text[max(0, start_idx-100):start_idx]
        is_rera_duplicate = "(" in bracket_context and "रेरा" in bracket_context and ")" not in bracket_context
        
        if not any(word in context_before for word in exclude_keywords) and not is_rera_duplicate:
            if 2.0 <= val < 900:
    ...
    if m_vals:
        if len(m_vals) > 1:
            ...
            # Tolerance 1.0 handles cases like 39.54 vs 38.91
